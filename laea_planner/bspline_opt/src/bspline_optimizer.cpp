...
const int BsplineOptimizer::SMOOTHNESS = (1 << 0);   // 1 1 (2^0)
const int BsplineOptimizer::DISTANCE = (1 << 1);
const int BsplineOptimizer::FEASIBILITY = (1 << 2);
const int BsplineOptimizer::START = (1 << 3);       // 1000 8 (2^3)
...
const int BsplineOptimizer::MINTIME = (1 << 8); // 0100000000 256 (2^8)
// add new
const int BsplineOptimizer::YAWING = (1 << 9);  // 1000000000 512 (2^9)
...
// 传参 cost_function_ -- 全局变量，将选择cost函数
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  cost_function_ = cost_code;

  // for yaw： int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::START | BsplineOptimizer::END | BsplineOptimizer::YAWING;
...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  // 传参 cost_function_ -- 全局变量，用于选择cost函数
  setCostFunction(cost_function);

  // Set necessary data and flag
  dim_ = control_points_.cols();  // .cols() 列数，yaw为1，选择3阶B样条
  if (dim_ == 1)
    order_ = 3;
  else
    order_ = bspline_degree_;
  point_num_ = control_points_.rows();
  // 优化时间 yawing: cost_function_ 数值 100011001 & 0100000000 = 0 ——> variable_num_= dim_ * point_num_
  // 要优化时间就多加一个变量？
...
void BsplineOptimizer::optimize() {
  ...
  // optimize -> costFunction -> combineCost(cost_function_ & YAWING) -> calcYawFeasibilityCost
...
// 该函数与 calcFeasibilityCost 一模一样，其利用 最大速度&加速度 计算路径的平滑cost 
// 此处替换为 max_dyaw_ max_ddyaw_， 即对应的 最大角速度和角加速度
...
void BsplineOptimizer::combineCost(const std::vector<double>& x, std::vector<double>& grad,
  ...
  // 根据 cost_function_ 选择添加相应的cost函数
...
// costFunction -> combineCost(cost_function_ & YAWING) -> calcYawFeasibilityCost
