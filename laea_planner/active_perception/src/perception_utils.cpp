...
void PerceptionUtils::getYawDir(const Vector3d& pos, const double& yaw, Vector3d& dir_normalized){
  ...
  Vector3d p_origin = Rwb * cam_vertices1_[0] + pos_;  // origin
  Vector3d p_left = Rwb * cam_vertices2_[0] + pos_;  // left_up 
  Vector3d p_right = Rwb * cam_vertices2_[2] + pos_;  // right_up

  dir_normalized = (p_left-p_origin) + (p_right-p_origin);
  // 忽略z轴
  dir_normalized(2) = 0;
  dir_normalized = dir_normalized.normalized();
  // dir_normalized(0) = (p_left(0)-p_origin(0)) + (p_right(0)-p_origin(0));
